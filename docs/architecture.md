...
两个半侧的生效方式不同：宿主半侧 `lib/index.js` 在 `dsh web` 启动时导入一次（改完要重启进程），浏览器半侧由 client-modules 每次从磁盘读给页面（刷新即可）。正因为生效时机不同，构建时会给两个半侧注入同一枚 `__LWGU_STAMP__`（`src/` 的源码指纹），宿主在每次响应里带上它，菜单据此显示「构建 xxxx」并在两侧不一致时提示重启——否则「新前端 + 旧后端」只会表现为一个费解的 404。
